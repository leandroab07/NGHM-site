...
import { getProjetos, getUsers } from '@/lib/data'
...
export async function POST(req: NextRequest) {
  ...
  const body: Projeto = await req.json()
  body.id = Date.now().toString()
  const { error } = await supabase.from('projetos').insert(body)
...
export async function PUT(req: NextRequest) {
  if (!await isAdmin(req)) return NextResponse.json({ error: 'Não autorizado' }, { status: 401 })
  const body: Projeto = await req.json()
  const { error } = await supabase.from('projetos').update(body).eq('id', body.id)
  if (error) return NextResponse.json({ error: error.message }, { status: 500 })
